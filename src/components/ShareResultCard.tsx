...
/** Detect if the document currently has dark mode active. */
function isDarkMode(): boolean {
  return document.documentElement.classList.contains('dark')
}

// Theme-aware color palettes for the canvas card
const themes = {
  dark: {
    bgStart: '#18181b',
    bgEnd: '#09090b',
...
export function ShareResultCard({
  ...
  const drawCard = useCallback(
    (canvas: HTMLCanvasElement | null) => {
      ...
      if (card.scaling) {
        y += 8
        ctx.fillStyle = card.scaling === 'Rx' ? t.rxGreen : t.scalingMuted
